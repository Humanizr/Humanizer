...
- KTD1. **Use Docusaurus 3.10 as the only site shell.** (session-settled: user-approved — chosen over Astro Starlight, Material for MkDocs, VitePress, and DocFX as the shell: Docusaurus provides the most mature native documentation-version lifecycle while remaining a static GitHub Pages build.) Use the classic preset, TypeScript 7 configuration, pnpm lockfile, built-in color modes, autogenerated sidebars where they remain readable, and the official Pages deployment pattern. Do not revive Jekyll or introduce a second site renderer.
...
- **Dependencies:** None.
- **Files:** `website/package.json`, `website/pnpm-lock.yaml`, `website/docusaurus.config.ts`, `website/sidebars.ts`, `website/docs/`, `.config/dotnet-tools.json`, `website/humanizer-versions.json`, `tools/docs/`.
- **Approach:**
  - Scaffold Docusaurus 3.10 with TypeScript 7, the classic preset, pnpm, a root landing page, and `/docs/` as the documentation route.
  ...
  - Exercise extension methods, overloads, generics, XML cross-references, deterministic output, local-source-path exclusion, and narrative-to-API relative links.
...
- **Verification:**
  ...
  - `pnpm --dir website run test:e2e --grep "navigation|theme|version|not found"`
...
- **Verification:**
  ...
  - `pnpm --dir website run check:content start scenarios concepts`
  - `pnpm --dir website run build`
...
- **Verification:**
  ...
  - `pnpm --dir website run check:content upgrading analyzer`
...
- **Verification:**
  ...
  - `pnpm --dir website run check:content languages contributing`
  - `pnpm --dir website run build`
...
- **Verification:**
  ...
  - `pnpm --dir website run test:a11y`
...
- **Dependencies:** U8.
- **Files:** `.github/workflows/docs.yml`, `.github/workflows/jekyll-gh-pages.yml`, `.github/dependabot.yml`, `.github/renovate.json`, `azure-pipelines.yml`, `website/`, `docs/_config.yml`, `docs/_layouts/`, `docs/assets/`, `docs/index.md`, contributor documentation.
- **Approach:**
  ...
  - Add hosted Renovate maintenance for the pnpm 11 site toolchain and cache both pnpm and immutable package/API inputs.
...
| Site dependencies | `pnpm --dir website install --frozen-lockfile` | U1, U3-U4, U7-U9 | Lockfile installs without mutation. |
...
| Static site | `pnpm --dir website run build` | U1, U3-U9 | Docusaurus reports no broken links, broken anchors, duplicate routes, or rendering errors. |
| Content quality | `pnpm --dir website run check:content` | U4-U6, U9 | Required orientation, examples, pitfalls, cross-links, persona, and page classification metadata exist. |
| Browser behavior | `pnpm --dir website run test:e2e` | U3, U7-U9 | Navigation, versioning, redirects, search, themes, missing routes, and mobile behavior pass. |
| Accessibility | `pnpm --dir website run test:a11y` | U3, U8-U9 | Representative pages in both themes have no serious or critical automated accessibility findings. |
| Search budget | `pnpm --dir website run check:search-budget` | U1, U7-U9 | Active search fetches one version only and no per-version index grows more than 25 percent without reviewed baseline approval. |
