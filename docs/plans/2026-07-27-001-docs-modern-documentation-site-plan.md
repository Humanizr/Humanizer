...
- XMLDoc2Markdown 6.0 or the declared DocFX Markdown fallback can consume release-specific assemblies and XML documentation without a sanitizer or second documentation renderer.
...
- [XMLDoc2Markdown 6.0](https://www.nuget.org/packages/XMLDoc2Markdown) — Docusaurus-ready Markdown generation from DLL/XML inputs.
...
- KTD6. **Use XMLDoc2Markdown 6.0 as the primary API generator with a direct-output gate.** (session-settled: user-approved — chosen over the prior DocFX plus DocFxMarkdownGen pipeline: its Docusaurus preset removes the need for a sanitizer and link rewriter.) Pin it in `.config/dotnet-tools.json`, request public members only, and ingest its Markdown as ordinary versioned documents. The gate covers extension methods, overload and generic anchors, `<inheritdoc>`, `<see>`, `<paramref>`, relative links, deterministic output, source-link behavior, and Docusaurus build/search ingestion. If it fails, DocFX's native CommonMark output is the declared fallback and must pass the same gate without DocFxMarkdownGen or post-processing.
- KTD7. **Use local build-time search with contextual versioning as the default.** (session-settled: user-directed — chosen over mixed-version default results: readers need answers for the release they selected.) Use the confirmed `@cmfcmf/docusaurus-search-local` dependency for active-version indexes and lazy loading. The all-version proof may add only a bounded theme extension that consumes public plugin and Docusaurus interfaces; it may not copy the plugin's SearchBar implementation. If the public interface cannot support that constraint, use the maintained `@easyops-cn/docusaurus-search-local` derivative's documented search contexts and all-context mode instead of vendoring search code.
...
- Keep custom React to the homepage, preview metadata wrapper, version-aware not-found page, and the bounded all-version search seam.
...
| Risk | Mitigation |
|---|---|
| API generator loses overload anchors, XML tags, or relative links | U1 uses representative Humanizer symbols and fails over to DocFX CommonMark only if it passes the identical no-sanitizer gate. |
...
- **Approach:**
  ...
  - Pin XMLDoc2Markdown 6.0, acquire every initial manifest package, and run a public API smoke generation against each declared DLL/XML/TFM pair; generate the full representative slice from `Humanizer.Core` `3.0.10` plus preview `Humanizer`.
  ...
  - Prove selected-version local search and the bounded all-version route without copying an upstream SearchBar.
