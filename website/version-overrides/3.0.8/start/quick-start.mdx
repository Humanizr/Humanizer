...
You will create a console app, install Humanizer `3.0.8`, and produce one deterministic duration. Use a .NET SDK that can create a project compatible with the selected package assets.
...
The assertion fails immediately if Humanizer `3.0.8` produces a different value.
...
This source is compiled and run against the selected Humanizer `3.0.8` package during documentation validation.
