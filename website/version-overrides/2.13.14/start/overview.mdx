...
The same small console program is compiled and run against the selected Humanizer `2.13.14` package during documentation validation:
...
- Need exact signatures: open the [2.13.14 API reference](../api/index.md).

## Documentation and support

This is the frozen documentation snapshot for Humanizer `2.13.14`. Documentation availability is not a promise that this release remains under active maintenance. Use the [issue tracker](https://github.com/Humanizr/Humanizer/issues) for confirmed bugs and support questions.
...
Package layout and target frameworks differ by release. Keep the version selector aligned with the Humanizer `2.13.14` package when following commands or API links on this page.
