...
`ByteSize` and `ByteRate` remain in `Humanizer.Bytes`. Culture-aware `ByteRate.Humanize` and string parsing with an explicit provider are available; span parsing is not.
