...
Run `dotnet list package --include-transitive`, inspect the target in `project.assets.json`, and select that Humanizer version in this site.
...
| `ToNumber` or `ByteSize.Parse` rejects input | Exact text and culture/provider | Use `TryToNumber` or `TryParse`; inspect the first unrecognized number token and byte unit suffix/case. |
...
| `CS8032` or `AD0001` from Humanizer analyzer | SDK/MSBuild and package version | Upgrade to `3.0.10`, clear stale build outputs, and restore. Do not add Roslyn dependencies to the application. |
...
| Search or a copied URL opens another API version | Current selector and URL | Re-select the version, then navigate from its API root. Narrative and API routes are versioned together. |
...
Package layout, target frameworks, analyzer assets, locale implementation, and API routes all vary across the supported corpus. `3.0.10` is the latest stable boundary. `main/preview` describes the checkout and is not named `3.5` or `4` until release authority assigns a version.
