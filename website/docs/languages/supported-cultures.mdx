...
here. An explicitly accepted regional culture can reuse a listed same-language
profile, so use `Configurator.IsCultureSupported` to check the requested
`CultureInfo` by exact name. Humanizer does not infer support by walking an
arbitrary .NET parent chain. Calendar or numeric formatting uses .NET
globalization data when a profile does not need a Humanizer override.
