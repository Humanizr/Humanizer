...
disagree, `main/preview` locale data can provide narrowly scoped calendar or
number-formatting overrides so Humanizer output stays stable.
...
Calendar and numeric overrides do not mutate `CultureInfo`. They apply only at
documented Humanizer call sites. Caller-supplied custom format providers remain
authoritative, and `ByteSize.TryParse` uses only the decimal-separator override
when an explicit culture provider is supplied.
...
The YAML-backed `calendar` and `number.formatting` override surfaces describe
`main/preview`. Historical releases can rely on different runtime or package
behavior, so use the selected snapshot and test on the target framework you
ship. `DateOnly` and `TimeOnly` APIs require a target framework on which those
types exist.
