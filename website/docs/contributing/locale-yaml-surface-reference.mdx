...
- Explicit SI and IEC formatting requires these entries and canonical symbols:
  ...
  Every non-variant locale must author localized forms for all 11 entries. The
  current catalog has 90 authoring roots and 12 regional variants. Variants
  may inherit the entries only through their existing same-language
  `variantOf` chain. A supported locale must never obtain these words from the
  default English formatter fallback.
