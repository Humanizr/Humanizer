...
- `minuteWordsMap`: optional locale-specific minute words indexed from 0 through 59. Use a dense sequence of at most 60 string entries; empty strings are permitted as placeholders. Use a sparse numeric mapping such as `1: 'one'` when only selected minute values need authored forms. Sparse keys must be unique integers from 0 through 59, and each sparse value must be a non-empty string. A variant's sparse entries override only those inherited minute slots; dense and sparse values share the same numeric-slot representation during inheritance.
