...
- `minuteWordsMap`: optional locale-specific minute words indexed from 0 through 59. Use a dense sequence for a complete table, or a sparse numeric mapping such as `1: 'one'` when only selected minute values need authored forms. Sparse mappings reject duplicate, non-integer, negative, and out-of-range keys.
