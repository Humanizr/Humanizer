...
10. `inflection` may inherit only from a locale with the same primary language
    subtag. A cross-language relationship such as `nn` → `nb` must author its
    own inflection profile.
