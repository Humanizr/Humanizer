...
| **After** | Automatic formatting selects PB/EB; `ByteRate` formats and compares normalized rates and implements comparison/equality interfaces. |
| **Symptom** | Large-size text changes, distinct instances collapse to one value key, or identical recompiled source selects a more specific interface overload. |
| **Minimal action** | Request an explicit TB format when contractual. Use an identity comparer or explicit cast only when identity or the old overload selection is deliberate. |
| **Verify** | Cover TB/PB/EB boundaries, `ByteRate.ToString`, equivalent rates, dictionary/set behavior, and overload selection after recompilation. |
| **Evidence** | Candidate [large-unit tests](https://github.com/Humanizr/Humanizer/blob/087cde3f5de637a2a8e6bbaacd7451a9f5f28e28/tests/Humanizer.Tests/Bytes/LargeDataUnitTests.cs#L32-L66), [comparison/equality tests](https://github.com/Humanizr/Humanizer/blob/087cde3f5de637a2a8e6bbaacd7451a9f5f28e28/tests/Humanizer.Tests/Bytes/ByteRateTests.cs#L149-L175), and the [`ByteRate` interface surface](https://github.com/Humanizr/Humanizer/blob/087cde3f5de637a2a8e6bbaacd7451a9f5f28e28/tests/Humanizer.Tests/ApiApprover/PublicApiApprovalTest.Approve_Public_Api.DotNet8_0.verified.txt#L6-L19); a focused stable/preview compile probe selects `object`/`IComparable` respectively. |
