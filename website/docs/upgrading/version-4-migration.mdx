...
When a culture argument is omitted or null, normal formatter and converter
paths now resolve from `CultureInfo.CurrentCulture`.
...
| Migration detail | What to do |
| --- | --- |
| **Affected callers** | Calls without an explicit culture in applications where formatting culture and UI resource culture differ. |
| **Before** | Default registries resolve normal formatting from `CurrentUICulture`. |
| **After** | Normal formatting resolves from `CurrentCulture`; only `ResolveForUiCulture()` explicitly uses `CurrentUICulture`. |
...
| **Verify** | Set the two ambient cultures to different values and exercise numbers, ordinals, relative dates, durations, collections, and clock notation. |
...
- **What's New:** the curated Humanizer 4 page is an integration dependency;
  its route is not present in the audited baseline and must be linked here when
  that task supplies a verified document ID.
