...
`TimeSpan.Humanize` is available across the documented corpus. `ToAge` begins in `3.0.1`. The verified executable targets `3.0.10` and current; older snapshots replace or exclude its age call.
...
- [Selected-version duration and age API](../api/index.md)
