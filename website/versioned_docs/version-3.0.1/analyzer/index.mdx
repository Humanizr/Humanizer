...
Pin the SDK used locally and in CI. Humanizer `3.0.1` ships the initial analyzer
layout; some Roslyn hosts can report `CS8032` or `AD0001`.
...
| `CS8032` or `AD0001` | This is the known `3.0.1` analyzer-host boundary; upgrade rather than patching analyzer dependencies into the application. |
...
`HUMANIZER001` first appears in `3.0.1` with default severity `error`. Later
patches change analyzer packaging and host compatibility.
