...
For ordinary applications using Humanizer `3.0.10`, install `Humanizer`.

| Documentation version | Normal install | Package shape |
| --- | --- | --- |
| `3.0.10` | `Humanizer` | Metapackage that brings in `Humanizer.Core` and locale packages |
...
Then use the `Humanizer` namespace. This verified program is compiled and run against the selected `3.0.10` package:

<CodeBlock language="csharp" title="Program.cs">{quickStart}</CodeBlock>

Choose `Humanizer.Core` directly only when you deliberately want the English-only core or must work around tooling that cannot restore the locale metapackage. Add only locale packages that exist for `3.0.10`.

## Pitfall

Do not copy `Humanizer.Core.<locale>` references from another release. Locale packaging is version-specific, and a package name is not proof that an API or language surface exists in `3.0.10`.
