...
`ToWords` supports `int` and `long` cardinal values. Word ordinals are `int`-based. Some overloads accept `GrammaticalGender` or `addAnd`; use named arguments so intent remains clear across overload changes.
...
and locale grammar have changed. The verified source runs against this selected
package. Historical snapshots link to their own capability data and signatures.
