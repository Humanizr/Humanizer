...
uses Windows NLS, while modern .NET normally uses ICU. When those sources disagree, test Humanizer `2.10.1` on every target platform the application supports.
...
This page is scoped to Humanizer `2.10.1`. Runtime globalization data can differ by target platform, so test on the target framework and operating systems you ship.
