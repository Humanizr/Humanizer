...
Pin the SDK used locally and in CI. Humanizer `3.0.8` contains versioned Roslyn
analyzer variants, but not the `3.0.10` fallback-selection fix.
...
| `CS8032` or `AD0001` on `3.0.1` | `3.0.8` adds versioned analyzer assets; do not patch dependencies into the application. |
| Duplicate analyzer/load behavior | Upgrade to `3.0.10` for the fallback-selection fix. |
...
`HUMANIZER001` retains default severity `error`. `3.0.8` introduces the
versioned analyzer layout; `3.0.10` later corrects fallback selection.
