...
The `3.0.8` `Humanizer` package is a metapackage over `Humanizer.Core` and locale packages. See [package selection](./package-selection.md) before replacing package references during an upgrade.
