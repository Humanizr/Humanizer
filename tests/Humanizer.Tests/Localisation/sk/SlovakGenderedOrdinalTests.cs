...
[UseCulture("sk")]
public class SlovakGenderedOrdinalTests
{
    static readonly CultureInfo Sk = new("sk");
    ...
    {
        Assert.Equal(expected, number.Ordinalize(gender, Sk));
        Assert.Equal(expected, number.ToString(Sk).Ordinalize(gender, Sk));
    }
    ...
    {
        var masculine = number.Ordinalize(GrammaticalGender.Masculine, Sk);
        var result = number.Ordinalize(gender, Sk);
