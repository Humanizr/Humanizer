...
[UseCulture("hr")]
public class CroatianGenderedOrdinalTests
{
    ...
    {
        Assert.Equal(expected, number.Ordinalize(gender, Hr));
        Assert.Equal(expected, number.ToString(Hr).Ordinalize(gender, Hr));
    }
    ...
    {
        var masculine = number.Ordinalize(GrammaticalGender.Masculine, Hr);
        var result = number.Ordinalize(gender, Hr);
    ...
    public void ToOrdinalWords_ProducesCroatianWords(int number, GrammaticalGender gender, string expected) =>
        Assert.Equal(expected, number.ToOrdinalWords(gender, WordForm.Normal, Hr));
}