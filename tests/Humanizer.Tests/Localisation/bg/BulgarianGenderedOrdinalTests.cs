...
[UseCulture("bg")]
public class BulgarianGenderedOrdinalTests
{
    ...
    {
        Assert.Equal(expected, number.Ordinalize(gender, Bg));
        Assert.Equal(expected, number.ToString(Bg).Ordinalize(gender, Bg));
    }
    ...
    {
        var masculine = number.Ordinalize(GrammaticalGender.Masculine, Bg);
        var result = number.Ordinalize(gender, Bg);
