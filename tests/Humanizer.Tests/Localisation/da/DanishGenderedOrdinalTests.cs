...
[UseCulture("da")]
public class DanishGenderedOrdinalTests
{
    static readonly CultureInfo Da = new("da");
    ...
    {
        Assert.Equal(expected, number.Ordinalize(gender, Da));
        Assert.Equal(expected, number.ToString(Da).Ordinalize(gender, Da));
    }
    ...
    {
        var masculine = number.Ordinalize(GrammaticalGender.Masculine, Da);
        var result = number.Ordinalize(gender, Da);
