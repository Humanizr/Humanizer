...
[UseCulture("cs")]
public class CzechGenderedOrdinalTests
{
    static readonly CultureInfo Cs = new("cs");
    ...
    {
        Assert.Equal(expected, number.Ordinalize(gender, Cs));
        Assert.Equal(expected, number.ToString(Cs).Ordinalize(gender, Cs));
    }
    ...
    {
        var masculine = number.Ordinalize(GrammaticalGender.Masculine, Cs);
        var result = number.Ordinalize(gender, Cs);
