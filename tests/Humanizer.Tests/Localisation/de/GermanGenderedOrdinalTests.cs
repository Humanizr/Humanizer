...
[UseCulture("de")]
public class GermanGenderedOrdinalTests
{
    static readonly CultureInfo De = new("de");
    ...
    {
        Assert.Equal(expected, number.Ordinalize(gender, De));
        Assert.Equal(expected, number.ToString(De).Ordinalize(gender, De));
    }
    ...
    {
        var masculine = number.Ordinalize(GrammaticalGender.Masculine, De);
        var result = number.Ordinalize(gender, De);
