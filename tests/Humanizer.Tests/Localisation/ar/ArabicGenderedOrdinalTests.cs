...
[UseCulture("ar")]
public class ArabicGenderedOrdinalTests
{
    static readonly CultureInfo Ar = new("ar");
    ...
    {
        Assert.Equal(expected, number.Ordinalize(gender, Ar));
        Assert.Equal(expected, number.ToString(Ar).Ordinalize(gender, Ar));
    }
    ...
    {
        var masculine = number.Ordinalize(GrammaticalGender.Masculine, Ar);
        var result = number.Ordinalize(gender, Ar);
