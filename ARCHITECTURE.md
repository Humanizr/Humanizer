...
| `LocaleRegistryInput` | Master locale registry (maps culture names to profiles) and `LocaleNumberFormattingOverrides.g.cs` (per-locale decimal separator, negative sign, and group separator overrides consumed by `OrdinalizeExtensions`, `ByteSize.ToString`, and `MetricNumeralExtensions`) |
