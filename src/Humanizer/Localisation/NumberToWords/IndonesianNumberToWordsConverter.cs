...
class IndonesianNumberToWordsConverter : GenderlessNumberToWordsConverter
{
    static readonly string[] UnitsMap = ["nol", "satu", "dua", "tiga", "empat", "lima", "enam", "tujuh", "delapan", "sembilan"];
    ...
    public override string Convert(long number)
    {
        ...
        if (number < 1_000_000_000_000)
        {
            ...
            var words = $"{Convert(number / 1_000_000_000)} miliar";
            return remainder == 0 ? words : $"{words} {Convert(remainder)}";
        }

        var trillionRemainder = number % 1_000_000_000_000;
        var trillionWords = $"{Convert(number / 1_000_000_000_000)} triliun";
