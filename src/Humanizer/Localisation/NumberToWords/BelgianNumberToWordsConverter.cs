...
{
    internal class BelgianNumberToWordsConverter : GenderedNumberToWordsConverter
    {
        ...
        private static readonly string[] TensMap = { "zéro", "dix", "vingt", "trente", "quarante", "cinquante", "soixante", "septante", "quatre-vingt", "nonante" };
        ...
        public override string Convert(int number, GrammaticalGender gender)
        {
            ...
            if (number > 0)
            {
                ...
                {
                    string lastPart = lastPart = TensMap[number / 10];
                    if ((number % 10) > 0)
                    {
                        if ((number - 1) % 10 == 0)
                            lastPart += " et un";
